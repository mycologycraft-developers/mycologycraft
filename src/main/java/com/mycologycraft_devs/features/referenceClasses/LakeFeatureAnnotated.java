...
public class LakeFeatureAnnotated extends Feature<LakeFeatureAnnotated.Configuration> {
	...
	public boolean place(FeaturePlaceContext<LakeFeatureAnnotated.Configuration> context) {
		...
		if (blockpos.getY() <= worldgenlevel.getMinBuildHeight() + 4) {
			...
			boolean[] airList = new boolean[2048];
			...
			for(int j = 0; j < i; ++j) {
				...
				double d3 = randomsource.nextDouble() * ((double)16.0F - d0 - (double)2.0F) + (double)1.0F + d0 / (double)4.0F;
				double d4 = randomsource.nextDouble() * ((double)8.0F - d1 - (double)4.0F) + (double)2.0F + d1 / (double)2.0F;
				double d5 = randomsource.nextDouble() * ((double)16.0F - d2 - (double)2.0F) + (double)1.0F + d2 / (double)4.0F;
			...
			for(int j = 0; j < i; ++j) {
				...
				double d1 = randomsource.nextDouble() * (double)4.0F + (double)2.0F;
				double d2 = randomsource.nextDouble() * (double)6.0F + (double)3.0F;
				double d3 = randomsource.nextDouble() * ((double)16.0F - d0 - (double)2.0F) + (double)1.0F + d0 / (double)8.0F;
				double d4 = randomsource.nextDouble() * ((double)8.0F - d1 - (double)4.0F) + (double)2.0F + d1 / (double)4.0F;
				double d5 = randomsource.nextDouble() * ((double)16.0F - d2 - (double)2.0F) + (double)1.0F + d2 / (double)8.0F;
				...
				for(int l = 1; l < 15; ++l) {
					for(int i1 = 1; i1 < 15; ++i1) {
						for(int j1 = 1; j1 < 7; ++j1) {
							...
							double d9 = d6 * d6 + d7 * d7 + d8 * d8;
							if (d9 < (double)4F) {
								outerMycelium[(l * 16 + i1) * 8 + j1] = true;
							}
						}
					}
				}
			...
			//i think this creates a barrier, gonna tinker here/*
			...
			BlockState mycelia = lakefeature$configuration.mycelia().getState(randomsource, blockpos);
			if (!blockstate2.isAir()) {
				for(int j2 = 0; j2 < 16; ++j2) {
					for(int j3 = 0; j3 < 16; ++j3) {
						for(int l3 = 0; l3 < 8; ++l3) {
							...
							if (!flag2) {
								BlockState blockstate = worldgenlevel.getBlockState(blockpos.offset(j2, l3, j3));
								BlockState above = worldgenlevel.getBlockState(blockpos.offset(j2, l3+1, j3));

								if (blockstate.isSolid() && !blockstate.is(BlockTags.LAVA_POOL_STONE_CANNOT_REPLACE) && !above.is(Blocks.CAVE_AIR) && !airList[(j2 * 16 + j3) * 8 + l3]) {
									BlockPos myceliumPos = blockpos.offset(j2, l3, j3);
									worldgenlevel.setBlock(myceliumPos, blockstate2, 2);
									this.markAboveForPostProcessing(worldgenlevel, myceliumPos);

								}
								if (blockstate.isSolid() && !blockstate.is(BlockTags.LAVA_POOL_STONE_CANNOT_REPLACE) && (above.is(Blocks.CAVE_AIR) || above.is(Blocks.AIR) || airList[(j2 * 16 + j3) * 8 + l3])) {
									BlockPos myceliumPos = blockpos.offset(j2, l3, j3);
									worldgenlevel.setBlock(myceliumPos, mycelia, 2);
									this.markAboveForPostProcessing(worldgenlevel, myceliumPos);
